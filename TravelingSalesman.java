...
public class TravelingSalesman
{
	...
	public static void main(String args[])
	{
		try
		{
			...
			TracerouteLoader importData = new TracerouteLoader("TraceRoutes.csv");
			importData.loadAddresses();
			//importData.generateModels();
			...
			tempCollection = importData.generateModels();
			...
			for(Cluster group: groups)
			{
				...
				if(input.length > 1)
				{
					TravelingSalesman tester = new TravelingSalesman(start, end, 45, 0.1f, input, 30f, initTime);
	...
	public TravelingSalesman(Geolocation s, Geolocation e, float hCW, float vOC, AssetModel coll[], float cS, long tT)
	{
		counter = 0;
